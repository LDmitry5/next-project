...
export default function Cart() {
  const { isOpen, cartItems, setIsOpen, deleteCartItem } = useCart();
      ...
      <div className="cart" style={{ display: isOpen ? "flex" : "none" }}>
        <div className="cart-body">
          ...
          <div className="cart-total">
            Общая сумма: <span>{cartItems.reduce((acc, item) => acc + item.price * item.count, 0)}</span> руб
          </div>
          ...
          <button className="btn btn-primary cart-confirm" onClick={() => setIsOpen(!isOpen)}>
            Оформить заказ
          </button>
